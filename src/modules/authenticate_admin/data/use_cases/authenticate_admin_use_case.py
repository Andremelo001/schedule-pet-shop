...
class AutheticateAdminUseCase(InterfaceAuthenticateAdminUseCase):
    ...
    async def generate_token_admin(self, senha: str, user: str) -> str:
        jwt_service = JWTService()

        await self.verify_user(senha, user)
        
        return jwt_service.create_token({"sub": senha, "role": "admin"})
    
    async def verify_user(self, senha: str, user: str) -> None:

        verify_senha = PasswordHasher()
        
        admin = await self.__repository.get_admin_by_user(user)
