...
class PetUpdateUseCase(InterfacePetUpdate):
    ...
    async def update(self, id_client: str, id_pet: str, pet: PetUpdateDTO) -> Dict:

        await self.__pet_exists(pet.name, id_client)

        new_pet = await self.__repository.uptdate_pet(id_pet, pet)

        return self.__format_response(new_pet)
    
    async def __pet_exists(self, pet_name: str, id_client: str) -> None:
