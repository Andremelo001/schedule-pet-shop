...
class ServiceUpdateUseCase(InterfaceServiceUpdate):
    ...
    async def update(self, service: UpdateServiceDTO, id_service: str) -> Dict:

        if service.type_service is not None:
            self.__validate_type_service(service.type_service)

        await self.__validate_service_in_schedule(id_service)

        await self.__service_not_found(id_service)

        new_service = await self.__repository.update_service(service, id_service)
    ...
    async def __validate_service_in_schedule(self, id_service: str) -> None:
