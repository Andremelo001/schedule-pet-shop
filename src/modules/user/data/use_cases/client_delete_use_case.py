...
class ClientDeleteUseCase(InterfaceClientDelete):
    ...
    async def delete(self, session: AsyncSession, id_client: str) -> Dict:
        ...
        await self.__verify_if_clients_has_schedules_actives(session, id_client)
    ...
    async def __verify_if_clients_has_schedules_actives(self, session: AsyncSession, id_client: str) -> None:

        schedules = await self.repository.find_schedule_by_id_client(session, id_client)
