...
async def test_create_pet_success(mocker):
    ...
    # Mock client exists in repository
    ...
    result = await use_case.create(pet_data, fake_client.id)

    assert result["message"] == "Pet cadastrado com sucesso"

    mock_repository.get_client_by_id.assert_called_once_with(fake_client.id)
    mock_repository.get_name_pet_by_id_client.assert_called_once_with(fake_client.id)
    mock_repository.create_pet.assert_called_once_with(fake_client.id, pet_data)
...
async def test_create_pet_client_not_found(mocker):
    ...
    with pytest.raises(HttpNotFoundError, match=f"Cliente com o id {client_id} não encontrado"):
        await use_case.create(pet_data, client_id)

    mock_repository.get_client_by_id.assert_called_once_with(client_id)
...
async def test_create_pet_name_conflict(mocker):
    ...
    with pytest.raises(HttpConflitError, match=f"Cliente com o id {fake_client.id}, já tem um pet cadastrado com o nome Rex"):
        await use_case.create(pet_data, fake_client.id)
