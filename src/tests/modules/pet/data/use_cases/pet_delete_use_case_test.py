...
async def test_delete_pet_success(mocker):
    ...
    result = await use_case.delete(mock_session, "pet-id-123")
    ...
    mock_repository.delete_pet.assert_called_once_with(mock_session, "pet-id-123")
...
async def test_delete_pet_not_found(mocker):
    ...
    with pytest.raises(HttpNotFoundError) as exc_info:
        await use_case.delete(mock_session, "pet-id-123")
