...
async def test_find_pet_success(mocker):
    ...
    result = await use_case.finder(mock_session, "pet-id-123")
    ...
    mock_repository.get_pet.assert_called_once_with(mock_session, "pet-id-123")
...
async def test_find_pet_not_found(mocker):
    ...
    with pytest.raises(HttpNotFoundError, match="Pet com o id pet-id-nonexistent não encontrado"):
        await use_case.finder(mock_session, "pet-id-nonexistent")

    mock_repository.get_pet.assert_called_once_with(mock_session, "pet-id-nonexistent")
