...
async def test_update_service_success(mocker):
    ...
    result = await use_case.update(update_data, "service-id-123")
    ...
    mock_repository.get_schedules_by_service_id.assert_called_once_with("service-id-123")
    mock_repository.find_service_by_id.assert_called_once_with("service-id-123")
    mock_repository.update_service.assert_called_once_with(update_data, "service-id-123")
...
async def test_update_service_not_found(mocker):
    ...
    with pytest.raises(HttpNotFoundError, match="Serviço não encontrado em nossa base de dados"):
        await use_case.update(update_data, "service-id-nonexistent")

    mock_repository.get_schedules_by_service_id.assert_called_once_with("service-id-nonexistent")
    mock_repository.find_service_by_id.assert_called_once_with("service-id-nonexistent")
...
async def test_update_service_invalid_type(mocker):
    ...
    with pytest.raises(HttpUnauthorized, match="Tipo de serviço não aceito, escolha um dos serviços disponíveis: banho, secar, tosa"):
        await use_case.update(update_data, "service-id-123")
