...
async def test_delete_client_success(mocker):
    ...
    result = await use_case.delete( "client-id-123")
    ...
    mock_repository.find_schedule_by_id_client.assert_called_once_with( "client-id-123")
    mock_repository.delete_client.assert_called_once_with( "client-id-123")
...
async def test_delete_client_not_found(mocker):
    ...
    with pytest.raises(HttpNotFoundError) as exc_info:
        await use_case.delete( "client-id-123")
